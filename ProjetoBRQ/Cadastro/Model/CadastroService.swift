                       ...
                       if let dadosRecebidos = dados {
                           do{
                               ...
                               if let arrayBancos = objetoJson["data"] as? Array<Any> {
                                   ...
                                   for i in (0...n-1) {
                                       if let dicionario = arrayBancos[i] as? [String:Any] {
                                           guard let banco = dicionario["nome"] as! String? else { return }
                                           guard let agencia = dicionario["agencia"] as! Int? else { return }
                                           guard let conta = dicionario["conta"] as! Int? else { return }
                                           guard let digito = dicionario["conta_dig"] as! Int? else { return }
                                           guard let id = dicionario["id"] else { return }
